// let serverIP = "b16adab1-b381-42ef-becb-c0d6cde93283-jolzz.tysonjenkins.dev";
// let serverPort = "443";
let serverIP = "0.0.0.0";
let serverPort = "3333";
let protocol = "ws";

const websocket = new WebSocket(`${protocol}://${serverIP}:${serverPort}`);

websocket.onopen = () => {
  console.info(`Opened WebSocket connection on ${protocol}://${serverIP}:${serverPort}`);
  websocket.send("u:jelly");
  // websocket.send("hello zig");
  // websocket.send("hello zig again");
  // websocket.send("What I'm doing?");
  // websocket.send("testing the inputs!");
}

websocket.onmessage = (event) => {
  console.log(event.data);
}

websocket.onclose = () => {
  console.log("closing");
}
